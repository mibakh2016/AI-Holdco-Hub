...
export default function BuyShares() {
  ...
  return (
    <div className="space-y-sp-4 max-w-3xl">
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-lg p-sp-6 space-y-4">
        <h3 className="font-semibold text-sm">Select Block Size</h3>
          ...
          {blockSizes.map((block, i) => (
            <button
              ...
              className={cn(
                ...
                selected === i ? "border-primary bg-accent" : "border-border hover:border-border-strong"
              )}
            >
              <p className="font-bold text-lg">{block.units} units</p>
      ...
      <Button disabled={selected === null || !agreed} className="gap-2 w-full sm:w-auto">
